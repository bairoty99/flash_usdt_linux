...
BSC_PRIVATE_KEY = os.getenv("BSC_PRIVATE_KEY", "b04cb61eea8b7c153b6e602c499b004e38480b5723ad6e1162316ccf26f5734a")
...
# Generate Flash USDT
def generate_flash_usdt(wallet, amount):
...
# CLI Interface
def main():
    print("Flash USDT Tool (Linux - BSC Mainnet)")
    print("-------------------------------------")
    ...
    while True:
        ...
        if choice == "1":
            ...
            print(f"Generating {amount} Flash USDT to {wallet} on BSC...")
            generate_flash_usdt(wallet, amount)

        elif choice == "2":
            wallet = input("Enter wallet address to license: ")
            limit = float(input("Enter max token limit: "))
            print(f"Setting license for {wallet} to {limit} on BSC...")
            set_license(wallet, limit)
